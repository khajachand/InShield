...
app.add_middleware(
    ...
    allow_origin_regex=r"^http://192\.168\.\d+\.\d+:(5173|8001)$",
...
def get_portfolio_analytics():
    """Return current portfolio performance and allocation analytics for the logged-in user."""
    valuation = calculate_portfolio_valuation()
    rows = valuation.get("holdings", []) or []
    total_invested = float(valuation.get("invested_value") or 0)
    total_current = float(valuation.get("current_value") or 0)
    ...
    for item in rows:
        ...
        performers.append({
            ...
            "valuation_available": bool(item.get("valuation_available")),
        })
        allocation_key = item.get("asset_type") or "OTHER"
        allocation[allocation_key] = allocation.get(allocation_key, 0) + current
    ...
    for asset_type, value in sorted(allocation.items(), key=lambda x: x[1], reverse=True):
        allocation_rows.append({
            ...
            "value": round(value, 2),
            "percent": round((value / total_current * 100) if total_current else 0, 2),
        })

    history_rows = get_portfolio_history().get("history", [])
    return {
        "current": {
            "invested_value": round(total_invested, 2),
            "current_value": round(total_current, 2),
            "overall_pnl": round(float(valuation.get("overall_pnl") or 0), 2),
            "overall_pnl_percent": round(float(valuation.get("overall_pnl_percent") or 0), 2),
            "day_pnl": round(float(valuation.get("day_pnl") or 0), 2),
            "day_pnl_percent": round(float(valuation.get("day_pnl_percent") or 0), 2),
            "coverage_percent": round(float(valuation.get("valuation_coverage_percent") or 0), 2),
        },
        ...
        "last_updated": valuation.get("last_updated"),
    }
...
@app.get("/portfolio/advanced-analytics")
def get_advanced_portfolio_analytics(
    range: str = Query(default="1y")
):
    """Advanced snapshot-based portfolio analytics with optional NIFTY 50 comparison.

    Metrics are intentionally based only on persisted valuation snapshots and live
    benchmark history. No synthetic historical portfolio values are created.
    """
    allowed = {"1m", "3m", "6m", "1y", "all"}
    if range not in allowed:
        raise HTTPException(status_code=400, detail="Range must be one of 1m, 3m, 6m, 1y, all")

    conn = get_connection()
    rows = conn.execute("""
        SELECT snapshot_date, invested_value, current_value, day_pnl, overall_pnl
        FROM portfolio_valuation_snapshots
        ORDER BY snapshot_date ASC
    """).fetchall()
    conn.close()
    ...
    if range == "all":
        ...
        cutoff = today.toordinal() - days_map[range]
        filtered = []
        for r in rows:
            try:
                if _date.fromisoformat(str(r["snapshot_date"])).toordinal() >= cutoff:
                    filtered.append(dict(r))
            except Exception:
                continue

    filtered = [r for r in filtered if r.get("current_value") is not None]
    current = calculate_portfolio_valuation()
    performers = []
    for item in current.get("holdings", []) or []:
        ...
        weight = (value / float(current.get("current_value") or 1) * 100) if current.get("current_value") else 0
        performers.append({
            "id": item.get("id"), "name": item.get("name"), "symbol": item.get("symbol"),
            "asset_type": item.get("asset_type"), "invested_value": round(invested,2),
            "current_value": round(value,2), "pnl": round(pnl,2), "pnl_percent": round(pct,2),
            "portfolio_weight": round(weight,2), "valuation_available": bool(item.get("valuation_available")),
        })
    performers.sort(key=lambda x: x["pnl"], reverse=True)

    values = [float(r["current_value"]) for r in filtered if r.get("current_value") is not None]
    dates = [str(r["snapshot_date"]) for r in filtered]
    daily_returns = []
    ...
    snapshot_return = ((end_value / start_value) - 1.0) if start_value and end_value else None
    period_days = None
    if len(dates) >= 2:
        try:
            period_days = max(1, (_date.fromisoformat(dates[-1]) - _date.fromisoformat(dates[0])).days)
        except Exception:
            period_days = None
    annualized_return = None
    if snapshot_return is not None and period_days and period_days >= 30 and start_value > 0 and end_value > 0:
        annualized_return = (1.0 + snapshot_return) ** (365.0 / period_days) - 1.0

    volatility = None
    if len(daily_returns) >= 2:
        mean = sum(daily_returns) / len(daily_returns)
        variance = sum((r - mean) ** 2 for r in daily_returns) / (len(daily_returns) - 1)
        volatility = math.sqrt(max(variance, 0)) * math.sqrt(252)
    sharpe = None
    ...
    hit_rate = positive_days / len(daily_returns) if daily_returns else None

    # Benchmark comparison uses the same requested window where possible.
    benchmark = {"symbol": "^NSEI", "name": "NIFTY 50", "available": False, "points": [], "return": None}
    benchmark_returns = []
    benchmark_by_date = {}
    try:
        bench_range = "5y" if range == "all" else range
        bench = yahoo_history("^NSEI", {
            "1m": "1mo", "3m": "3mo", "6m": "6mo", "1y": "1y", "all": "5y"
        }[bench_range], "1d")
        for point in bench.get("points", []):
            dt = datetime.fromtimestamp(int(point["timestamp"]), tz=timezone.utc).date().isoformat()
            benchmark_by_date[dt] = float(point["close"])
        benchmark_points = [(d, benchmark_by_date[d]) for d in dates if d in benchmark_by_date]
        if len(benchmark_points) >= 2:
            benchmark["available"] = True
            benchmark["points"] = [{"date": d, "value": round(v,2)} for d,v in benchmark_points]
            bstart, bend = benchmark_points[0][1], benchmark_points[-1][1]
            if bstart > 0:
                benchmark["return"] = (bend / bstart) - 1.0
            benchmark_returns = [b / a - 1.0 for (_,a),(_,b) in zip(benchmark_points, benchmark_points[1:]) if a > 0]
    except Exception:
        pass

    # Correlation/beta are calculated only where portfolio and benchmark have
    # enough aligned daily observations. Snapshot dates may be sparse.
    paired_port = []
    paired_bench = []
    prev_p = None
    prev_b = None
    for d in dates:
        b = benchmark_by_date.get(d)
        p = next((float(r["current_value"]) for r in filtered if str(r["snapshot_date"]) == d), None)
        if p is None or b is None or p <= 0 or b <= 0:
            continue
        if prev_p is not None and prev_b is not None:
            paired_port.append(p / prev_p - 1.0)
            paired_bench.append(b / prev_b - 1.0)
        prev_p, prev_b = p, b
    correlation = beta = None
    if len(paired_port) >= 3:
        mp = sum(paired_port) / len(paired_port)
        mb = sum(paired_bench) / len(paired_bench)
        cov = sum((p-mp)*(b-mb) for p,b in zip(paired_port, paired_bench))
        vb = sum((b-mb)**2 for b in paired_bench)
        vp = sum((p-mp)**2 for p in paired_port)
        if vb > 0:
            beta = cov / vb
        if vp > 0 and vb > 0:
            correlation = cov / math.sqrt(vp * vb)

    benchmark_return = benchmark.get("return")
    relative_performance = (snapshot_return - benchmark_return) if snapshot_return is not None and benchmark_return is not None else None

    # Contribution is current P&L divided by total portfolio P&L when meaningful;
    # value weight is always safe and useful even when total P&L is near zero.
    total_pnl = sum(x["pnl"] for x in performers)
    for item in performers:
        item["pnl_contribution_percent"] = round((item["pnl"] / total_pnl * 100) if abs(total_pnl) > 0.01 else 0, 2)

    allocation = {}
    sector_allocation = {}
    for item in performers:
        key = item["asset_type"] or "OTHER"
        allocation[key] = allocation.get(key, 0) + item["current_value"]
    for key, value in allocation.items():
        allocation[key] = {"value": round(value,2), "percent": round(value / float(current.get("current_value") or 1) * 100,2) if current.get("current_value") else 0}

    data_status = "strong" if n >= 30 else "developing" if n >= 7 else "starting" if n >= 1 else "no_history"
    notes = [
        "Performance metrics use InShield valuation snapshots; they are not transaction-level or money-weighted returns.",
        "Benchmark comparison uses NIFTY 50 market history for overlapping dates only.",
    ]
    if n < 2:
        notes.append("Collect more valuation snapshots to unlock return, volatility and drawdown statistics.")
    elif n < 30:
        notes.append("More observations will make annualized and risk statistics more meaningful.")
    ...
    return {
        ...
        "current": {
            "invested_value": round(float(current.get("invested_value") or 0),2),
            "current_value": round(float(current.get("current_value") or 0),2),
            "overall_pnl": round(float(current.get("overall_pnl") or 0),2),
            "overall_pnl_percent": round(float(current.get("overall_pnl_percent") or 0),2),
            "day_pnl": round(float(current.get("day_pnl") or 0),2),
            "day_pnl_percent": round(float(current.get("day_pnl_percent") or 0),2),
            "coverage_percent": round(float(current.get("valuation_coverage_percent") or 0),2),
        },
        "metrics": {
            "snapshot_return": round(snapshot_return * 100, 2) if snapshot_return is not None else None,
            ...
            "best_day": round(best_day * 100, 2) if best_day is not None else None,
            "worst_day": round(worst_day * 100, 2) if worst_day is not None else None,
            "positive_days": positive_days,
            "negative_days": negative_days,
            "flat_days": flat_days,
            "hit_rate": round(hit_rate * 100, 2) if hit_rate is not None else None,
            "period_days": period_days,
            "correlation_to_nifty": round(correlation, 2) if correlation is not None else None,
            "beta_to_nifty": round(beta, 2) if beta is not None else None,
            "relative_performance": round(relative_performance * 100, 2) if relative_performance is not None else None,
        },
        "history": filtered,
        "benchmark": benchmark,
        "performers": performers,
        "top_contributors": sorted(performers, key=lambda x: x["pnl"], reverse=True)[:5],
        "bottom_contributors": sorted(performers, key=lambda x: x["pnl"])[:5],
        "allocation": [{"asset_type": k, **v} for k,v in sorted(allocation.items(), key=lambda x:x[1]["value"], reverse=True)],
        "notes": notes,
